...
function assertConversionResultRoot(result) {
  ...
  assert.strictEqual(result.converter, 'text2markdown')
...
async function main() {
  ...
  try {
    ...
    const base = `http://127.0.0.1:${port}/api/text-to-markdown`

    allPassed =
      (await runScenario('success: plain text paragraph', async () => {
        const response = await fetch(base, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ text: 'Simple line.\n\nSecond line.' }),
        })
    ...
    allPassed =
      (await runScenario('success: list-like text input', async () => {
        const response = await fetch(base, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    allPassed =
      (await runScenario('failure: empty text input (EMPTY_INPUT)', async () => {
        const response = await fetch(base, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    allPassed =
      (await runScenario('failure: zero-length text string (EMPTY_INPUT)', async () => {
        const response = await fetch(base, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ text: '' }),
  ...
  console.log('[OK] representative e2e scenarios validated for /api/text-to-markdown')
}

main()
  .then(() => exitClean(process.exitCode || 0))
  .catch(async (error) => {
    console.error('[FAIL] representative scenario verification failed for /api/text-to-markdown')
    console.error(error && error.stack ? error.stack : String(error))
    await exitClean(1)
  })
