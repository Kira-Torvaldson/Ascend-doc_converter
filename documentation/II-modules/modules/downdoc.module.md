...
Le module `downdoc` est un wrapper pour la bibliothèque JavaScript downdoc qui convertit les documents AsciiDoc en Markdown. Ce module implémente l'interface définie dans [modules.interface.md](../modules.interface.md) et respecte les obligations minimales de sécurité de la version 1.
...
La méthode `run` effectue la conversion d'un fichier AsciiDoc en Markdown selon le processus suivant :
...
- Le module peut appliquer des options de conversion si elles sont fournies dans le paramètre `options`
- Les options prises en charge peuvent inclure :
  - Mode de conversion (standard ou compatible BookStack/Parsedown)
  - Extensions spécifiques (parsedown, etc.)

#### 4. Post-traitement du résultat

- Le module applique un nettoyage basique au Markdown généré pour corriger les problèmes courants de la bibliothèque downdoc
- Les corrections incluent notamment :
  - Correction des règles horizontales mal formées (`- --` → `---`)
...
- Le champ `logs` doit contenir les journaux d'exécution (début, étapes, fin)
...
- **`inputPath`** (obligatoire) : Chemin absolu vers le fichier AsciiDoc d'entrée
...
- **Journaux d'exécution** : Le module doit produire des journaux décrivant les étapes principales (lecture, conversion, écriture)
...
Le module prend en charge un mode de conversion spécial pour BookStack, qui applique des adaptations supplémentaires pour garantir la compatibilité avec le parseur Parsedown utilisé par BookStack. Ce mode est activé via l'option `mode: 'bookstack'`.
