 ...
 * @param {string|Buffer} [options.stdinData] - Piped to the child's stdin (avoids temp files)
...
function safeSpawn(executable, args, options = {}) {
  ...
  const hasStdinData = options.stdinData !== undefined && options.stdinData !== null

  return new Promise((resolve, reject) => {
    const child = spawn(executable, args, {
      cwd: options.cwd,
      shell: false,
      stdio: [hasStdinData ? 'pipe' : 'ignore', 'pipe', 'pipe']
    })

    if (hasStdinData) {
      // EPIPE can happen if the child exits before reading all input;
      // the close handler will surface the real outcome.
