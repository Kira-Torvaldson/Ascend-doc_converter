# Options Supplémentaires de Normalisation

## Vue d'ensemble

Ce module définit les options avancées de normalisation qui améliorent la cohérence, la stabilité, la compatibilité inter-formats et la sécurité liée aux caractères et à l'encodage.

**Principe fondamental** : Aucune option active par défaut ne doit altérer le sens du texte.

## Structure de Configuration

### UnicodeManagement (Gestion Unicode avancée)

Gère le support Unicode et la normalisation des caractères.

- **`mode`** (`'full'|'restricted'|'disabled'`)
  - **`'full'`** : Support complet de tous les caractères Unicode valides (recommandé)
  - **`'restricted'`** : Limite aux plages Unicode spécifiées
  - **`'disabled'`** : Désactive le support Unicode avancé
  - Valeur par défaut : `'full'`
  - **Impact** : Détermine quels caractères Unicode sont acceptés. Le mode `'full'` est recommandé pour la compatibilité maximale.

- **`normalization`** (`'none'|'NFC'|'NFKC'`)
  - **`'none'`** : Aucune normalisation
  - **`'NFC'`** : Forme Canonique Composée (recommandé, ne modifie pas le sens)
  - **`'NFKC'`** : Forme Canonique Composée de Compatibilité (plus agressive, peut modifier certains caractères)
  - Valeur par défaut : `'NFC'`
  - **Impact** : La normalisation NFC garantit une représentation cohérente des caractères (ex: é peut être représenté comme un seul caractère ou e+accent). NFC est non-destructif, NFKC peut modifier certains caractères.

- **`detectConfusables`** (`boolean`)
  - Détecte les caractères visuellement confusables (ex: cyrillique vs latin)
  - Valeur par défaut : `true`
  - **Impact** : Détecte les caractères qui peuvent être confondus visuellement, utile pour la sécurité (détection d'attaques d'homoglyphes).
...
- **`removeDirectionalChars`** (`boolean`)
  - Supprime les caractères directionnels (RTL/LTR marks)
  - Valeur par défaut : `false` (désactivé pour préserver l'affichage)
  - **Impact** : Évite les problèmes d'affichage dans certains outils, mais peut altérer l'affichage de textes bidirectionnels. Désactivé par défaut.
...
- **`preserveWhitespace`** (`boolean`)
  - Préserve les espaces essentiels (tabs, newlines)
  - Valeur par défaut : `true`
...
- **`enableTransliteration`** (`boolean`)
  ...
  - **Impact** : Peut altérer le sens du texte. Désactivé par défaut.
...
- **`unicodeToAscii`** (`UnicodeToAsciiFallback`)
  ...
  - **Impact** : Permet de convertir des caractères Unicode en ASCII, mais peut être destructif. Désactivé par défaut.
...
- **`rejectInvalidSequences`** (`boolean`)
  - Refuse les séquences Unicode invalides
  - Valeur par défaut : `true`
  - **Impact** : Important pour la sécurité et la stabilité. Active par défaut.

- **`rejectPrivateChars`** (`boolean`)
  - Refuse les caractères privés (Private Use Area, 0xE000-0xF8FF)
...
- **`warnOutOfRange`** (`boolean`)
  - Signale les caractères hors plage autorisée
  - Valeur par défaut : `true`
...
- **`continueOnWarning`** (`boolean`)
  - Continue le traitement malgré les warnings
  - Valeur par défaut : `true`
...
- Détection des confusables avec warnings uniquement
...
**Utilisation** : Cas d'usage général, conversion de documents standards. Garantit la cohérence sans altérer le sens.

### StrictAdvancedNormalizationOptions (Environnements critiques)

- Mode Unicode complet
- Normalisation NFC
- Détection des confusables avec warnings
...
- `ZERO_WIDTH_CHARS` : Caractères de largeur zéro
...
const userOptions = {
  unicode: {
    normalization: 'NFC', // Forcer NFC
  },
  characterCleaning: {
    removeControlChars: true, // Activer le nettoyage
  },
  processingMode: {
...
2. **Nettoyage** : Désactiver par défaut (`removeControlChars: false`, etc.) sauf si nécessaire. Activer uniquement si vous êtes sûr que cela ne supprimera pas de contenu légitime.

3. **Translittération** : Désactiver par défaut (`strategy: 'none'`) pour préserver le sens. Activer uniquement si la conversion ASCII est explicitement requise.
...
- ✅ Les warnings sont utilisés plutôt que les modifications automatiques
- ✅ La validation rejette uniquement les séquences invalides (sécurité)

**Important** : Activer le nettoyage ou la translittération peut altérer le sens. Toujours vérifier avec `checkOptionsSafety()` avant d'activer ces options.

## Notes de Sécurité

- Les caractères de largeur zéro peuvent être utilisés pour des attaques d'injection. Le nettoyage est recommandé si vous traitez du contenu non fiable.
- Les caractères confusables (homoglyphes) peuvent être utilisés pour des attaques de phishing. La détection avec warnings est recommandée.
